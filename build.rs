...
fn write_tables() {
    ...
    if cfg!(any(feature = "simd-neon", feature = "simd-ssse3", feature = "simd-avx2", feature = "simd-avx512", feature = "simd-gfni")) {
...
#[cfg(any(feature = "simd-neon", feature = "simd-ssse3", feature = "simd-avx2", feature = "simd-avx512", feature = "simd-gfni"))]
...
#[cfg(not(any(feature = "simd-neon", feature = "simd-ssse3", feature = "simd-avx2", feature = "simd-avx512", feature = "simd-gfni")))]
fn compile_simd_c() {}
...
fn generate_encode_codegen_avx2(f: &mut File, configs: &[(usize, usize)]) {
    ...
    writeln!(f, "// Auto-generated AVX2 encode functions for common configurations.").unwrap();
    ...
    writeln!(f, "    not(any(target_os = \"android\", target_os = \"ios\"))").unwrap();
    writeln!(f, "))]").unwrap();
    ...
    for &(d, p) in configs {
        ...
        writeln!(f, "            avx2_impl::encode_{d}x{p}_avx2(parity_rows, data, parity, shard_len);").unwrap();
...
fn generate_encode_fn_avx2(f: &mut File, d: usize, p: usize) {
    ...
    writeln!(f, "    /// Specialized encode for {d} data + {p} parity shards using AVX2.").unwrap();
    writeln!(f, "    ///").unwrap();
    writeln!(f, "    /// # Safety").unwrap();
    writeln!(f, "    ///").unwrap();
    writeln!(f, "    /// Caller must ensure AVX2 is available and all slices have length >= shard_len.").unwrap();
    ...
    writeln!(f, "        let nibble_mask: __m256i = _mm256_set1_epi8(0x0f);").unwrap();

    // Load coefficient tables for each (parity_row, data_shard) pair
    writeln!(f, "        // Load GF multiplication table halves for all coefficients.").unwrap();
    writeln!(f, "        // Layout: coef_tables[p_idx][d_idx] = (low_tbl, high_tbl)").unwrap();
    for pi in 0..p {
        for di in 0..d {
            writeln!(f, "        let (coef_low_{pi}_{di}, coef_high_{pi}_{di}): (__m256i, __m256i) = {{").unwrap();
            writeln!(f, "            let c = parity_rows[{pi}][{di}];").unwrap();
            writeln!(f, "            let (lh, hh) = super::super::load_table_halves(c);").unwrap();
            writeln!(f, "            let low128: __m128i = _mm_loadu_si128(lh.as_ptr().cast());").unwrap();
            writeln!(f, "            let high128: __m128i = _mm_loadu_si128(hh.as_ptr().cast());").unwrap();
    ...
    writeln!(f).unwrap();
    writeln!(f, "        // Main SIMD loop: process 32 bytes per iteration.").unwrap();
    ...
    for pi in 0..p {
        ...
        writeln!(f, "            let low = _mm256_and_si256(d0, nibble_mask);").unwrap();
        writeln!(f, "            let high = _mm256_and_si256(_mm256_srli_epi64::<4>(d0), nibble_mask);").unwrap();
        writeln!(f, "            let mut acc_{pi}: __m256i = _mm256_xor_si256(").unwrap();
        writeln!(f, "                _mm256_shuffle_epi8(coef_low_{pi}_0, low),").unwrap();
        writeln!(f, "                _mm256_shuffle_epi8(coef_high_{pi}_0, high),").unwrap();
        writeln!(f, "            );").unwrap();

        // For remaining data shards: XOR accumulate
        for di in 1..d {
            writeln!(f, "            let low = _mm256_and_si256(d{di}, nibble_mask);").unwrap();
            writeln!(f, "            let high = _mm256_and_si256(_mm256_srli_epi64::<4>(d{di}), nibble_mask);").unwrap();
            writeln!(f, "            acc_{pi} = _mm256_xor_si256(acc_{pi}, _mm256_xor_si256(").unwrap();
            writeln!(f, "                _mm256_shuffle_epi8(coef_low_{pi}_{di}, low),").unwrap();
            writeln!(f, "                _mm256_shuffle_epi8(coef_high_{pi}_{di}, high),").unwrap();
            writeln!(f, "            ));").unwrap();
        }

        writeln!(f, "            _mm256_storeu_si256(parity[{pi}][offset..].as_mut_ptr().cast(), acc_{pi});").unwrap();
    }
...
fn generate_encode_codegen_neon(f: &mut File, configs: &[(usize, usize)]) {
    ...
    writeln!(f, "// Auto-generated NEON encode functions for common configurations.").unwrap();
    writeln!(f, "// Generated by build.rs — do not edit manually.").unwrap();
    writeln!(f).unwrap();
    writeln!(f, "#[cfg(all(").unwrap();
    ...
    for &(d, p) in configs {
        writeln!(f, "        ({d}, {p}) => unsafe {{").unwrap();
        writeln!(f, "            neon_impl::encode_{d}x{p}_neon(parity_rows, data, parity, shard_len);").unwrap();
        writeln!(f, "            true").unwrap();
        writeln!(f, "        }},").unwrap();
    }
    writeln!(f, "        _ => false,").unwrap();
...
fn generate_encode_fn_neon(f: &mut File, d: usize, p: usize) {
    ...
    writeln!(f, "    /// Specialized encode for {d} data + {p} parity shards using NEON.").unwrap();
    writeln!(f, "    ///").unwrap();
    writeln!(f, "    /// # Safety").unwrap();
    writeln!(f, "    ///").unwrap();
    writeln!(f, "    /// Caller must ensure NEON is available and all slices have length >= shard_len.").unwrap();
    ...
    for pi in 0..p {
        for di in 0..d {
            ...
            writeln!(f, "            let c = parity_rows[{pi}][{di}];").unwrap();
            writeln!(f, "            (vld1q_u8(super::super::super::MUL_TABLE_LOW[c as usize].as_ptr()),").unwrap();
            writeln!(f, "             vld1q_u8(super::super::super::MUL_TABLE_HIGH[c as usize].as_ptr()))").unwrap();
            writeln!(f, "        }};").unwrap();
        }
    }
    ...
    writeln!(f).unwrap();
    writeln!(f, "        // Main SIMD loop: process 16 bytes per iteration.").unwrap();
    writeln!(f, "        let mut offset = 0usize;").unwrap();
    writeln!(f, "        while offset < bytes_done {{").unwrap();
    ...
    // Load all data shards
    for di in 0..d {
        writeln!(f, "            let d{di}: uint8x16_t = vld1q_u8(data[{di}][offset..].as_ptr());").unwrap();
    }
    ...
    for pi in 0..p {
        ...
        writeln!(f, "            vst1q_u8(parity[{pi}][offset..].as_mut_ptr(), acc_{pi});").unwrap();
    }
