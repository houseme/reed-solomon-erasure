...
unsafe fn rust_vsx_mul_slice_impl(c: u8, input: &[u8], out: &mut [u8]) {
    use core::arch::powerpc64::{__vector_unsigned_char, vec_and, vec_perm, vec_sr, vec_xor};

    let low_tbl: __vector_unsigned_char =
        core::mem::transmute(super::super::MUL_TABLE_LOW[c as usize]);
    let high_tbl: __vector_unsigned_char =
        core::mem::transmute(super::super::MUL_TABLE_HIGH[c as usize]);
    let nibble_mask: __vector_unsigned_char = vec_splats(0x0fu8);
    ...
    {
        let i0: __vector_unsigned_char = core::ptr::read_unaligned(in_chunk.as_ptr().cast());
        let i1: __vector_unsigned_char =
            core::ptr::read_unaligned(in_chunk.as_ptr().add(16).cast());
        let i2: __vector_unsigned_char =
...
unsafe fn rust_vsx_mul_slice_xor_impl(c: u8, input: &[u8], out: &mut [u8]) {
        ...
        .zip(unrolled_out.chunks_exact_mut(64))
    {
        ...
        let o2: __vector_unsigned_char =
            core::ptr::read_unaligned(out_chunk.as_ptr().add(32).cast());
        let o3: __vector_unsigned_char =
