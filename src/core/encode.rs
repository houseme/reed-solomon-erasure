...
mod tests {
    ...
    type Galois8ReedSolomon = ReedSolomon<Field>;

    const GENERATED_CONFIGS: &[(usize, usize)] =
    ...
    fn deterministic_data(data_shard_count: usize, shard_len: usize) -> Vec<Vec<u8>> {
        (0..data_shard_count)
            .map(|shard_index| {
                (0..shard_len)
                    .map(|byte_index| {
    ...
    fn encode_generic(codec: &Galois8ReedSolomon, data: &[Vec<u8>], parity: &mut [Vec<u8>]) {
    ...
    fn generated_encode_matches_generic_across_layout_and_tail_matrix() {
        for &(data_shard_count, parity_shard_count) in GENERATED_CONFIGS {
            let codec = Galois8ReedSolomon::new(data_shard_count, parity_shard_count).unwrap();

            for &shard_len in SIMD_BOUNDARY_LENGTHS {
                let data = deterministic_data(data_shard_count, shard_len);
                ...
                encode_generic(&codec, &data, &mut generic_parity);
    ...
    fn four_plus_two_recovers_all_supported_missing_combinations() {
        let codec = Galois8ReedSolomon::new(4, 2).unwrap();
        let data = deterministic_data(4, 65);
        let mut parity = vec![vec![0; 65]; 2];
        encode_generic(&codec, &data, &mut parity);
